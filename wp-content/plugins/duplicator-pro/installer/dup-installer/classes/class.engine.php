...
class DUPX_UpdateEngine
{
    ...
    public static function replaceBlogsAndSiteTables()
    {
        $s3Funcs  = DUPX_S3_Funcs::getInstance();
        $nManager = DUPX_NOTICE_MANAGER::getInstance();
        $muMode   = $s3Funcs->getPost('action_mu_mode');

        if ($muMode == DUPX_MultisiteMode::Subdomain || $muMode == DUPX_MultisiteMode::Subdirectory) {
            $dbh         = $s3Funcs->getDbConnection();
            $muSearch    = $s3Funcs->getPost('mu_search');
            $muReplace   = $s3Funcs->getPost('mu_replace');
            $mainSubsite = $GLOBALS['DUPX_AC']->subsites[0];
            $mainUrl     = $mainSubsite->name;

            DUPX_Log::resetIndent();
            DUPX_Log::info("ENGINE UPDATE BLOGS TABLE\n");

            foreach ($GLOBALS['DUPX_AC']->subsites as $cSub) {
                if ($s3Funcs->isMapping() && isset($muSearch[$cSub->id])) {
                    $search = $muSearch[$cSub->id];
                } else {
                    $search = $cSub->name;
                }

                if ($s3Funcs->isMapping() && isset($muReplace[$cSub->id])) {
                    $replace = $muReplace[$cSub->id];
                } else {
                    $replace = DUPX_U::getDefaultURL($cSub->name, $mainUrl, $muMode === DUPX_MultisiteMode::Subdomain);
                }
                ...
                // UPDATE `rte4ed_blogs` SET `domain` = 'www.temptest.loca', `path` = '/subsite3a/' WHERE `rte4ed_blogs`.`blog_id` = 3;
                $table = '`'.$GLOBALS['DUPX_AC']->wp_tableprefix.'blogs`';

                $sql = 'UPDATE '.$table.' SET `domain` = \''.$rHost.'\', `path` = \''.$rPath.'\' WHERE '.$table.'.`blog_id` = '.$cSub->id;
                if (@mysqli_query($dbh, $sql) === false) {
                    $errMsg = 'blogs table updates error'.
                        "\n\t".mysqli_error($dbh).
                        "\n\tTABLE:".$table.' HOST: '.$rHost.' PATH: '.$rPath.' SUBSITEID: '.$cSub->id;

                    $s3Funcs->report['errsql'][] = ($GLOBALS['LOGGING'] == DUPX_Log::LV_DEFAULT) ? 'DB ERROR: '.$errMsg : 'DB ERROR: '.$errMsg."\nSQL: [{$sql}]\n";
                    $nManager->addFinalReportNotice(array(
                        'shortMsg' => 'DATA-REPLACE ERRORS: MySQL',
                    ...
                    DUPX_Log::info('ENGINE BLOG TALBE UPDATED ID:'.$cSub->id.' DOMAIN:'.$rHost.' PATH:'.$rPath, DUPX_Log::LV_DETAILED);
                }
            }
        }
    }
