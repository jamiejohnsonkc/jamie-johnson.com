    ...
    public function rescan_remote_folder_set_backup($remote_id,$backups)
    {
        ...
        if(!empty($backups['remote']))
        {
            foreach ($backups['remote'] as $id=>$backup)
            {
                ...
                if(!file_exists($wpvivid_plugin->wpvivid_log->GetSaveLogFolder().$id.'_backup'.'_log.txt'))
                {
                    $wpvivid_plugin->wpvivid_log->CreateLogFile($id.'_backup','no_folder','backup');
                ...
                {
                    $backup_data['log']=$wpvivid_plugin->wpvivid_log->GetSaveLogFolder().$id.'_backup'.'_log.txt';
                }
