...
class WPvivid_Custom_Interface_addon
{
    ...
    public static function wpvivid_exclude_files_default($type, $files, $plugin_path){
        ...
        foreach ($files as $file) {
            ...
            if($type == 'Core'){
                ...
                if($dir == self::transfer_path(WP_CONTENT_DIR.DIRECTORY_SEPARATOR.WPvivid_Setting::get_backupdir(), '/')){
                    $exclude = true;
                }
    ...
    public function wpvivid_get_exclude_uploads_list(){
        $exclude_path = self::get_custom_backup_setting();
        $ret = '';
        if(!empty($exclude_path['uploads_option']['exclude_uploads_list'])) {
            foreach ($exclude_path['uploads_option']['exclude_uploads_list'] as $index => $value) {
                $ret .= '<ul>
                            <li>
