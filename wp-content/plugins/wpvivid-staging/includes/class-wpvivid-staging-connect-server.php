...
class WPvivid_Staging_Connect_server
{
    ...
    public function login($email,$user_info,$encrypt_user_info,$use_token=false,$get_key=false)
    {
        if($get_key)
            $public_key='';
        else
        ...
        if($encrypt_user_info)
        {
            if($use_token)
            {
                $encrypt_user_info=$crypt->encrypt_user_token($email,$user_info);
                $encrypt_user_info=base64_encode($encrypt_user_info);
            }
            else
            {
                $encrypt_user_info=$crypt->encrypt_user_info($email,$user_info);
                $encrypt_user_info=base64_encode($encrypt_user_info);
            }
        ...
        if(!is_wp_error($request) && ($request['response']['code'] == 200))
        {
            ...
            if(isset($body['token']))
            {
                $encrypt_user_info=$crypt->encrypt_user_token($email,$body['token']);
                $encrypt_user_info=base64_encode($encrypt_user_info);
                $info['email']=$email;
    ...
    public function update_pro($email,$user_info)
    {
        $public_key=get_option('wpvivid_connect_key','');
        if(empty($public_key))
        {
        ...
        if(!is_wp_error($request) && ($request['response']['code'] == 200))
        {
            ...
            {
                $path=WPVIVID_STAGING_PLUGIN_DIR.DIRECTORY_SEPARATOR.$params['file_name'];
                @unlink($path);
                ...
                $zip_ret = $archive->extract(PCLZIP_OPT_PATH,dirname($path),PCLZIP_OPT_REPLACE_NEWER,PCLZIP_OPT_REMOVE_PATH,'wpvivid-staging');
