...
class WPvivid_Staging_Copy_DB_Ex
{
    public $task;
    ...
    public function do_copy_db()
    {
        ...
        while($table&&!$table['finished'])
        {
            ...
            if(!$table['finished'])
            {
                $wpvivid_staging->log->WriteLog('Copying '.$this->task->get_db_insert_count().' queries is completed.','notice');
                $wpvivid_staging->log->WriteLog('The next copying table info: '.json_encode($table),'notice');
            }
        ...
        {
            $wpvivid_staging->log->WriteLog('Copying '.$table['name'].' is completed.','notice');
            $this->task->update_calc_db_finish_size('db');
            $wpvivid_staging->log->WriteLog('Database copying is completed.','notice');
    ...
    public function do_replace_db()
    {
        ...
        while($table&&!$table['finished'])
        {
            ...
            if(!$table['finished'])
            {
                $wpvivid_staging->log->WriteLog($this->task->get_db_replace_count().' queries of '.$table['name'].' is replaced.','notice');
                $wpvivid_staging->log->WriteLog('The next replacing table info: '.json_encode($table),'notice');
            }

            $this->task->update_table('db_replace',$table);
        }

        $wpvivid_staging->log->WriteLog('Replacing '.$table['name'].' is completed.','notice');
        ...
        if($this->task->get_start('db_replace')===false)
        {
            ...
            //$this->set_staging_site_data();
            $wpvivid_staging->log->WriteLog('Replacing database is completed.','notice');
    ...
    public function do_rename_db()
    {
        ...
        while($table)
        {
            ...
            $wpvivid_staging->log->WriteLog('Rename '.$table['name'].' is completed.','notice');
        ...
        $this->set_staging_site_data();
        $wpvivid_staging->log->WriteLog('Rename tables is completed.','notice');
    ...
    public function init_replace_tables_list()
    {
        ...
        {
            global $wpvivid_staging;
            $wpvivid_staging->log->WriteLog('Retrieve the tables required to replace.','notice');
    ...
    public function init_copy_tables_list()
    {
        ...
        {
            global $wpvivid_staging;
            $wpvivid_staging->log->WriteLog('Retrieve the tables required to copy.','notice');
            ...
            global $wpdb;
            $all_tables = (array) $wpdb->get_results( $wpdb->prepare( "SHOW FULL TABLES" ), ARRAY_N );
    ...
    public function init_rename_tables_list()
    {
        ...
        {
            global $wpvivid_staging;
            $wpvivid_staging->log->WriteLog('Retrieve the tables required to rename.','notice');
    ...
    public function copy_table_data(&$table,$count)
    {
        ...
        if($this->is_same_database())
        {
            ...
            if($new_db->query( "INSERT INTO `{$new_table_name}` ".$select )===false)
            {
                ...
                global $wpvivid_staging;
                $wpvivid_staging->log->WriteLog('new start offset '.$start,'warning');
    ...
    public function replace_table(&$table,$count)
    {
        ...
        if(substr($table['name'], strlen($this->replace_prefix))=='usermeta')
        {
            if($this->old_prefix!=$this->new_prefix)
            {
                ...
                $wpvivid_staging->log->WriteLog($update_query, 'notice');
                $wpvivid_staging->log->WriteLog('The length of UPDATE statement: '.strlen($update_query), 'notice');
                if ($db->get_results($update_query)===false)
                {
                    $error=$db->last_error;
                    $wpvivid_staging->log->WriteLog($error, 'Warning');
        ...
        if(is_multisite())
        {
            ...
            {
                $wpvivid_staging->log->WriteLog('update mu blogs', 'notice');
                ...
                if((preg_match('#^https?://([^/]+)#i', $this->new_home_url, $matches) || preg_match('#^https?://([^/]+)#i', $this->new_site_url, $matches)) && (preg_match('#^https?://([^/]+)#i', $this->old_home_url, $old_matches) || preg_match('#^https?://([^/]+)#i', $this->old_site_url, $old_matches)))
                {
                    ...
                    if($result && sizeof($result)>0)
                    {
                        ...
                        $wpvivid_staging->log->WriteLog(json_encode($mu_option), 'notice');
                        foreach ($rows as $row)
                        {
                            $update=array();
                            $where=array();
        ...
        {
            $wpvivid_staging->log->WriteLog('Ignore table '.$table['name'], 'Warning');
            $table['finished']=1;
            return $ret;
        }
        ...
        if($sum>0)
        {
            ...
            $wpvivid_staging->log->WriteLog('Start replacing the table prefix from '.$start_row. ' row.', 'notice');
            $end_row=$count;
            ...
            if($result && sizeof($result)>0)
            {
                ...
                foreach ($rows as $row)
                {
                    ...
                    foreach ($columns as $column)
                    {
                        ...
                        {
                            $wpvivid_staging->log->WriteLog('Skip MEDIUMBLOB data.', 'notice');
                            continue;
                        }
        ...
        $wpvivid_staging->log->WriteLog('Replacing database tables is completed.', 'notice');
        $table['start'] += $count;
        if( $table['start'] > $sum )
        {
            $table['finished']=1;
    ...
    private function rename_table(&$table)
    {
        ...
        $wpvivid_staging->log->WriteLog('DROP TABLE '.$og_table,'notice');
        if($db->query("DROP TABLE IF EXISTS {$og_table}")===false)
        {
            $wpvivid_staging->log->WriteLog('Failed to drop table '.$og_table.', error:'.$db->last_error,'notice');
        }
        $wpvivid_staging->log->WriteLog('Rename a table named '.$table_name.' to '.$og_table.' ','notice');

        if ($db->query("RENAME TABLE {$table_name} TO {$og_table}")===false)
        {
            $wpvivid_staging->log->WriteLog('Failed to rename a table named '.$table_name.' to '.$og_table.', error: '.$db->last_error,'notice');
        }
