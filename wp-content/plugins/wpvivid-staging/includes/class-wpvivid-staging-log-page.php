...
class WPvivid_Staging_Log_List extends WP_List_Table
{
    public $page_num;
    public $log_list;
    ...
    public function column_wpvivid_log_action( $log )
    {
        $html='<a class="open-log" log="'.$log['file_name'].'" style="cursor:pointer;">
                  <img src="'.esc_url(WPVIVID_STAGING_PLUGIN_URL.'/includes/images/Log.png').'" style="vertical-align:middle;">Log
               </a>';
        echo $html;
    }
    ...
    public function column_wpvivid_download( $log )
    {
        $html='<a class="download-log" log="'.$log['file_name'].'" style="cursor:pointer;">
                    <img src="' . esc_url(WPVIVID_STAGING_PLUGIN_URL . '/includes/images/download.png') . '" style="vertical-align:middle;" />Download
               </a>';
        echo $html;
    }
...
class WPvivid_Staging_Log_Page
{
    public $main_tab;
    ...
    public function init_page()
    {
        ...
        <div class="wrap" style="max-width:1720px;">
            ...
            $this->main_tab->add_tab('Log','log',array($this, 'output_log_page'));
        ...
        <script>
            jQuery(document).ready(function ()
            {
                ...
                {
                ?>
                jQuery( document ).trigger( '<?php echo $this->main_tab->container_id ?>-show',[ 'log', 'debug' ]);
                <?php
                }
                ?>
            });
    ...
    public function output_log_page()
    {
        ?>
        <div class="postbox quickbackup-addon">
            <div>
                <div class="wpvivid-element-space-bottom wpvivid-element-space-right" style="float: left;">
                    <img src="<?php echo esc_url(WPVIVID_STAGING_PLUGIN_URL.'includes/images/logs.png'); ?>" style="width:50px;height:50px;">
                </div>
                <div class="wpvivid-element-space-bottom">
                    <div class="wpvivid-text-space-bottom" style="margin-bottom: 0;">The tab displays the log of every backup, restoration, staging and other task.</div>
                </div>
            ...
            $this->main_tab->add_tab('Staging Log','staging_log',array($this, 'output_staging_log_list'), $args);

            $args['can_delete']=1;
            $args['hide']=1;
            $this->main_tab->add_tab('Log','open_log',array($this, 'output_log'), $args);
            $this->main_tab->display();
        ...
        <script>
            ...
            {
                location.href =ajaxurl+'?_wpnonce='+wpvivid_ajax_object.ajax_nonce+'&action=wpvividstg_download_log&log='+log;
            }

            function wpvivid_log_change_page(page,type,log_contenter)
            {
                var ajax_data = {
                    'action':'wpvividstg_get_log_list_page',
                    'page': page,
                    'type':type
                };
    ...
    public function output_log_list($type,$slug,$id)
    {
        ...
        <div class="wpvivid-log-list" id="<?php echo $id?>">
            ...
            $table = new WPvivid_Staging_Log_List();
