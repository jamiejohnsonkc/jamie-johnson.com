...
class WPvivid_Staging_Log
{
    ...
    {
        if(!is_dir(WP_CONTENT_DIR.DIRECTORY_SEPARATOR.WPVIVID_STAGING_PATH))
        {
            @mkdir(WP_CONTENT_DIR.DIRECTORY_SEPARATOR.WPVIVID_STAGING_PATH,0777,true);
            @fopen(WP_CONTENT_DIR.DIRECTORY_SEPARATOR.WPVIVID_STAGING_PATH.DIRECTORY_SEPARATOR.'index.html', 'x');
            $tempfile=@fopen(WP_CONTENT_DIR.DIRECTORY_SEPARATOR.WPVIVID_STAGING_PATH.DIRECTORY_SEPARATOR.'.htaccess', 'x');
            if($tempfile)
            {
                $text="deny from all";
        ...
        return WP_CONTENT_DIR.DIRECTORY_SEPARATOR.WPVIVID_STAGING_PATH.DIRECTORY_SEPARATOR;
    }
    ...
    public function WriteLogHander()
    {
        if ($this->log_file_handle)
        {
            ...
            if(isset($options['max_execution_time']))
            {
                ...
                $max_execution_time=WPVIVID_STAGING_MAX_EXECUTION_TIME_EX;
            }
...
class WPvivid_Staging_error_log
{
