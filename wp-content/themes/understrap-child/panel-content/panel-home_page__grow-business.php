<?php
/**
* container Splash template.
*
* @package understrap
*/
if ( ! defined( 'ABSPATH' ) ) {
exit; // Exit if accessed directly.
}
?>
	
 

<article class="article__wrapper homepage-claim__wrapper">
	<!-- <div class="homepage-claim__hackfix homepage-claim__hackfix--upper"></div> -->
	<div class="homepage-claim__background-image--container">
		<img src="//jamiejohnsonmev2.test/wp-content/uploads/2019/04/arrow-up.svg" class="style-svg arrow-up-svg" alt="...">
	</div>
	<div class="homepage-claim__container homepage-claim__container--content-wrapper">
			<h1 class="pagination pagination--homepage pagination--claim pagination--overlay ">What I Do</h1>
		
		<div class="homepage-claim__container homepage-claim__container--content">

		<!-- <div class="homepage-summary__claim_subhead homepage-summary__claim_subhead--weak">I DON'T DO OVERHEAD</div>
		<div class="homepage-summary__claim_subhead homepage-summary__claim_subhead--strong  homepage-summary__claim_subhead--border">I CREATE OUTCOMES</div> -->
		<div class="headline headline__header title title--overlay title--claim">I help companies optimize marketing for performance and growth.</div>
		<!-- <p class="homepage-summary__claim--fancy">I help companies optimize marketing for performance and growth.</p> -->
	</div>
		<!-- understand, engage & optimize marketing for performance and growth -->



	</div>
			<a class="link link__chevron homepage-claim__link" href="http://jamiejohnsonmev2.test/modus-operandi#i-advance-business-objectives" title="go to next">
			<img src="http://jamiejohnsonmev2.test/wp-content/uploads/2018/12/chevdn.svg" alt="..." class="chev--dwn style-svg" id="link__advance_chev-dwn"/>
		</a>
			<!-- <div class="homepage-claim__hackfix homepage-claim__hackfix--lower"></div> -->
</article>
